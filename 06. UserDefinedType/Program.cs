...
namespace _06._UserDefinedType
{
	internal class Program
	{
		 ...
		 * 튜플 (Tuple)
		 * 
		 * 간단한 데이터 구조에서 여러 데이터 요소를 그룹화하는 간결한 구문
		 ****************************************************************/

		// <튜플 기본사용>
		static void Main4()
		{
			(double, int) tuple1 = (4.5, 3);    // double과 int 데이터를 튜플로 보관
			Console.WriteLine("{0}, {1}", tuple1.Item1, tuple1.Item2);          // 기본 튜플의 데이터요소는 Item으로 접근

			(double doubleData, int intData) tuple2 = (4.5, 3);                 // Item 이름을 직접 정의
			Console.WriteLine("{0}, {1}", tuple2.intData, tuple2.doubleData);   // Item 이름을 정의한 경우 정의한 이름으로 사용
			Console.WriteLine(tuple2);          // (6.3, 3) : tuple을 출력할 경우 ()안에 데이터요소들을 표현

			(double, double) tuple3 = tuple2;   // 형변환이 가능한 튜플에 대입 가능
												// (int, double) tuple4 = tuple2;	// error : 형변환이 불가능한 튜플에 대입 불가
		...
		static void Main6()
		{
			...
			// Point point5 = new Point(10);											// error : 구현하지 않은 초기화
		}
